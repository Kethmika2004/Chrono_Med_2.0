...
let recast_parsers_babel_ts_js = require("recast/parsers/babel-ts.js");
recast_parsers_babel_ts_js = require_runtime.__toESM(recast_parsers_babel_ts_js);
...
const buildNodeForValue = (val, existingNode, fallbackLocale, requiredImports) => {
	...
	if (nodeType && [
	...
	].includes(nodeType) && nodeType !== _intlayer_types_nodeType.TEXT) {
		...
		else if (nodeType === _intlayer_types_nodeType.MARKDOWN) calleeName = "md";
		else if (nodeType === _intlayer_types_nodeType.HTML) calleeName = "html";
		else if (nodeType === _intlayer_types_nodeType.FILE) calleeName = "file";
		else if (nodeType === _intlayer_types_nodeType.NESTED) calleeName = "nest";
...
const transformJSFile = async (fileContent, dictionary, fallbackLocale, noMetadata) => {
	...
	let ast;
	try {
		ast = recast.parse(fileContent, { parser: recast_parsers_babel_ts_js });
	} catch (error) {
		console.error({ error });
		return fileContent;
	}
