...
const buildSystemFields = (baseDir, customConfiguration) => {
	...
	const optionalJoinBaseDir = (pathInput) => {
		let absolutePath;
		try {
			absolutePath = getProjectRequire(projectBaseDir).resolve(pathInput, { paths: [projectBaseDir] });
		} catch {
...
const buildContentFields = (systemConfig, customConfiguration) => {
	...
	const optionalJoinBaseDir = (pathInput) => {
		let absolutePath;
		try {
			absolutePath = getProjectRequire(systemConfig.baseDir).resolve(pathInput, { paths: [systemConfig.baseDir] });
		} catch {
			try {
