...
const buildSystemFields = (baseDir, customConfiguration) => {
	...
	const optionalJoinBaseDir = (pathInput) => {
		let absolutePath;
		try {
			absolutePath = require_utils_ESMxCJSHelpers.getProjectRequire(projectBaseDir).resolve(pathInput, { paths: [projectBaseDir] });
		} catch {
...
const buildContentFields = (systemConfig, customConfiguration) => {
	...
	const optionalJoinBaseDir = (pathInput) => {
		let absolutePath;
		try {
			absolutePath = require_utils_ESMxCJSHelpers.getProjectRequire(systemConfig.baseDir).resolve(pathInput, { paths: [systemConfig.baseDir] });
		} catch {
			try {
