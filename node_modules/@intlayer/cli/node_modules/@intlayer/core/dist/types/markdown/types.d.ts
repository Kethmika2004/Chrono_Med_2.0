...
type MarkdownContext<T> = {
  /** Framework runtime (createElement, etc.) */runtime: MarkdownRuntime; /** Component components for HTML tags */
  components?: ComponentOverrides<T>; /** Custom URL sanitizer */
  sanitizer?: (url: string, tag: HTMLTag, attr: string) => string | null; /** Custom slugify function for heading IDs */
  slugify?: (text: string, defaultFn: (input: string) => string) => string; /** Custom named codes to unicode mappings */
...
type ParseState = {
  ...
  key?: string | number; /** true if in a list */
...
type Rule<ParserOutput = ParserResult> = {
  /** Match function - returns regex match or null */_match: (source: string, state: ParseState, prevCapturedString?: string) => RegExpMatchArray | null; /** Priority order for rule application */
  _order: number; /** Parse function - converts regex capture to AST node */
...
type CompileOptions<T = any> = MarkdownOptions & {
  /** Component components */components?: ComponentOverrides<T>; /** Custom named codes to unicode mappings */
  namedCodesToUnicode?: Record<string, string>; /** Custom sanitizer function */
  sanitizer?: (value: string, tag: HTMLTag, attribute: string) => string | null; /** Custom slugify function */
