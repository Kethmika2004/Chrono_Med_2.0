...
const fill = async (options) => {
	...
	if (options?.build === true) await (0, _intlayer_chokidar_build.prepareIntlayer)(configuration, { forceRun: true });
	else if (typeof options?.build === "undefined") await (0, _intlayer_chokidar_build.prepareIntlayer)(configuration);
	...
	*/
	...
	const globalLimiter = (0, _intlayer_chokidar_utils.getGlobalLimiter)(nbConcurrentTranslations);
	const taskLimiter = (0, _intlayer_chokidar_utils.getTaskLimiter)(Math.max(1, Math.min(options?.nbConcurrentTasks ?? nbConcurrentTranslations, translationTasks.length)));
	const runners = translationTasks.map((task) => taskLimiter(async () => {
		...
		if (typeof effectiveFill === "string" || typeof effectiveFill === "function" || typeof effectiveFill === "object" && effectiveFill !== null) await require_fill_writeFill.writeFill({
			...
			await (0, _intlayer_chokidar_build.writeContentDeclaration)(dictionaryOutput, configuration);
			if (dictionaryOutput.filePath) appLogger(`${task.dictionaryPreset} Content declaration written to ${(0, _intlayer_chokidar_utils.formatPath)((0, node_path.basename)(dictionaryOutput.filePath))}`, { level: "info" });
		}
	}));
