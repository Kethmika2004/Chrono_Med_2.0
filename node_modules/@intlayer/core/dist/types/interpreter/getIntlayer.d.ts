import { DeepTransformContent, IInterpreterPluginState, Plugins } from "./getContent/plugins.js";
import { DeclaredLocales, DictionaryKeys, DictionaryRegistryContent, LocalesValues } from "@intlayer/types/module_augmentation";

//#region src/interpreter/getIntlayer.d.ts
declare const getIntlayer: <const T extends DictionaryKeys, const L extends LocalesValues = DeclaredLocales>(key: T, locale?: L, plugins?: Plugins[]) => DeepTransformContent<DictionaryRegistryContent<T>, IInterpreterPluginState, L>;
//#endregion
export { getIntlayer };
//# sourceMappingURL=getIntlayer.d.ts.map