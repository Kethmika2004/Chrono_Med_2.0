...
const deepMergeContent = (target, source, locale) => {
	...
	if (target && typeof target === "object" && target.nodeType === _intlayer_types_nodeType.TRANSLATION) if (locale) {
...
const processNewContent = (content, locale) => {
	...
	if (content && typeof content === "object" && content.nodeType === _intlayer_types_nodeType.TRANSLATION) return content;
	...
	if (locale) return {
		...
		[_intlayer_types_nodeType.TRANSLATION]: { [locale]: content }
	};
	else return content;
};
/**
