...
const intlayerToI18nextPlugin = {
	...
	transform: (node, props, next) => {
		...
		if (node.nodeType === _intlayer_types_nodeType.INSERTION) {
			if (node[_intlayer_types_nodeType.INSERTION].match(/\{[^}]*,[^}]*\}/)) return node[_intlayer_types_nodeType.INSERTION];
