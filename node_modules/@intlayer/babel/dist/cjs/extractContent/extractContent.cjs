...
const processFileInternal = (filePath, packageName, options, dependencies, saveComponent, providedComponentKey) => {
	...
	const compilerCommonOptions = {
		...
		shouldExtract: require_extractContent_utils_shouldExtract.shouldExtract,
		attributesToExtract: require_extractContent_utils_constants.ATTRIBUTES_TO_EXTRACT,
	...
	if (ext === ".astro") {
		...
		if (saveComponent) {
			...
			const formatCommand = (0, _intlayer_chokidar_cli.detectFormatCommand)(configuration);
...
const handleExtractionSideEffects = async (extractedContentMap, filePath, options, { configuration, baseDir, appLogger }, saveComponent) => {
	...
	if (!options?.codeOnly && !options?.onExtract) for (const [key, content] of Object.entries(extractedContentMap)) {
		const relativeContentFilePath = (0, node_path.relative)(baseDir, await require_extractContent_contentWriter.writeContentHelper(content, key, filePath, configuration));
...
const extractContent = async (filePath, packageName, options) => {
	...
	const dictionaryKey = require_extractContent_utils_extractDictionaryKey.extractDictionaryKey(filePath, options?.code ?? (0, node_fs.readFileSync)(filePath, "utf-8"), configuration.compiler.dictionaryKeyPrefix);
