...
const intlayerToVueI18nPlugin = {
	canHandle: (node) => {
		...
		if (Array.isArray(node)) {
			...
			for (const item of node) if (typeof item === "string") {} else if (item && typeof item === "object" && (item.nodeType === NodeTypes.INSERTION || item.nodeType === NodeTypes.ENUMERATION || item.nodeType === NodeTypes.GENDER || item.nodeType === "composite")) hasNode = true;
