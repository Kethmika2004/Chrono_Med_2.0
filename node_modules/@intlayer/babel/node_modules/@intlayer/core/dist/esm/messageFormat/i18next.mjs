...
const intlayerToI18nextPlugin = {
	canHandle: (node) => {
		...
		if (Array.isArray(node)) {
			...
			for (const item of node) if (typeof item === "string") {} else if (item && typeof item === "object" && (item.nodeType === NodeTypes.INSERTION || item.nodeType === NodeTypes.HTML || item.nodeType === NodeTypes.ENUMERATION || item.nodeType === NodeTypes.GENDER || item.nodeType === "composite")) hasNode = true;
